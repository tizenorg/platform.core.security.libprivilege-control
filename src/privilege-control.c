				       ...
				       bool set_transmutable)
{
	...
	if (set_transmutable) {
		...
		ret = dir_set_smack_r(path, "1", SMACK_LABEL_TRANSMUTE,
				      label_dirs);
		if (PC_OPERATION_SUCCESS != ret) {
...
static int perm_app_setup_path_internal(const char* pkg_id, const char* path, app_path_type_t app_path_type, va_list ap UNUSED)
{
