...
/*
 * Generic definitions for three domain policy model.
 * If they are found in some general package then they should be removed from
 * here. Maybe they (or some of them) should go to tizen-platform-config or
 * tizen-platform-wrapper?
 */
#define TD_LABEL_USER	"User"
/* End of definitions for three domains (TD). */
 ...
 * 	  and its content with one common access label ("User: TD_LABEL_USER), and set it
 * 	  to be transmutable. All applications will have full access to that directory.
 *
 ...
 * It must be called by privileged user.
 *
 * @param  pkg_id         application identifier or "User" (TD_LABEL_USER) if there is no
 *                        reason to add permissions for this app to assign full permissions
 *                        to "User" label
 * @param  path           file or directory path
 * @param  app_path_type  application path type
 * @param  shared_label   optional argument for APP_PATH_ANY_LABEL; type is const char*
 * @return                PC_OPERATION_SUCCESS on success, PC_ERR_* on error
 */
